...
const CURLM_ABORTED_BY_CALLBACK = 11 % Int32
const CURLM_UNRECOVERABLE_POLL = 12 % Int32
const CURLM_LAST = 13 % Int32
...
const CURLSSLBACKEND_RUSTLS = 14 % UInt32
...
function Base.getproperty(x::Ptr{__JL_Ctag_148}, f::Symbol)
...
function Base.getproperty(x::__JL_Ctag_148, f::Symbol)
    r = Ref{__JL_Ctag_148}(x)
    ptr = Base.unsafe_convert(Ptr{__JL_Ctag_148}, r)
    fptr = getproperty(ptr, f)
    GC.@preserve r unsafe_load(fptr)
end

function Base.setproperty!(x::Ptr{__JL_Ctag_148}, f::Symbol, v)
...
function Base.getproperty(x::Ptr{curl_fileinfo}, f::Symbol)
    ...
    f === :strings && return Ptr{__JL_Ctag_148}(x + 56)
...
const curl_prereq_callback = Ptr{Cvoid}
...
const CURLE_SETOPT_OPTION_SYNTAX = 49 % UInt32
...
const CURLE_OBSOLETE62 = 62 % UInt32
...
const CURLE_SSL_CLIENTCERT = 98 % UInt32
const CURLE_UNRECOVERABLE_POLL = 99 % UInt32
const CURL_LAST = 100 % UInt32
...
function Base.getproperty(x::Ptr{curl_hstsentry}, f::Symbol)
    f === :name && return Ptr{Ptr{Cchar}}(x + 0)
    f === :namelen && return Ptr{Csize_t}(x + 8)
    f === :includeSubDomains && return (Ptr{Cuint}(x + 16), 0, 1)
    f === :expire && return Ptr{NTuple{18, Cchar}}(x + 20)
    return getfield(x, f)
end

function Base.getproperty(x::curl_hstsentry, f::Symbol)
    r = Ref{curl_hstsentry}(x)
    ptr = Base.unsafe_convert(Ptr{curl_hstsentry}, r)
    fptr = getproperty(ptr, f)
    begin
        if fptr isa Ptr
...
function Base.setproperty!(x::Ptr{curl_hstsentry}, f::Symbol, v)
...
mutable struct curl_index
    index::Csize_t
    total::Csize_t
    curl_index() = new()
end

const CURLSTScode = UInt32
const CURLSTS_OK = 0 % UInt32
const CURLSTS_DONE = 1 % UInt32
const CURLSTS_FAIL = 2 % UInt32

# typedef CURLSTScode ( * curl_hstsread_callback ) ( CURL * easy , struct curl_hstsentry * e , void * userp )
const curl_hstsread_callback = Ptr{Cvoid}

# typedef CURLSTScode ( * curl_hstswrite_callback ) ( CURL * easy , struct curl_hstsentry * e , struct curl_index * i , void * userp )
const curl_hstswrite_callback = Ptr{Cvoid}
...
const CURLOPT_HSTS_CTRL = 299 % UInt32
const CURLOPT_HSTS = 10300 % UInt32
const CURLOPT_HSTSREADFUNCTION = 20301 % UInt32
const CURLOPT_HSTSREADDATA = 10302 % UInt32
const CURLOPT_HSTSWRITEFUNCTION = 20303 % UInt32
const CURLOPT_HSTSWRITEDATA = 10304 % UInt32
const CURLOPT_AWS_SIGV4 = 10305 % UInt32
const CURLOPT_DOH_SSL_VERIFYPEER = 306 % UInt32
const CURLOPT_DOH_SSL_VERIFYHOST = 307 % UInt32
const CURLOPT_DOH_SSL_VERIFYSTATUS = 308 % UInt32
const CURLOPT_CAINFO_BLOB = 40309 % UInt32
const CURLOPT_PROXY_CAINFO_BLOB = 40310 % UInt32
const CURLOPT_SSH_HOST_PUBLIC_KEY_SHA256 = 10311 % UInt32
const CURLOPT_PREREQFUNCTION = 20312 % UInt32
const CURLOPT_PREREQDATA = 10313 % UInt32
const CURLOPT_MAXLIFETIME_CONN = 314 % UInt32
...
struct __JL_Ctag_147
    data::NTuple{8, UInt8}
end

function Base.getproperty(x::Ptr{__JL_Ctag_147}, f::Symbol)
    f === :whatever && return Ptr{Ptr{Cvoid}}(x + 0)
    f === :result && return Ptr{CURLcode}(x + 0)
    return getfield(x, f)
end

function Base.getproperty(x::__JL_Ctag_147, f::Symbol)
    r = Ref{__JL_Ctag_147}(x)
    ptr = Base.unsafe_convert(Ptr{__JL_Ctag_147}, r)
    fptr = getproperty(ptr, f)
    GC.@preserve r unsafe_load(fptr)
end

function Base.setproperty!(x::Ptr{__JL_Ctag_147}, f::Symbol, v)
    unsafe_store!(getproperty(x, f), v)
end
...
function Base.getproperty(x::Ptr{CURLMsg}, f::Symbol)
    ...
    f === :data && return Ptr{__JL_Ctag_147}(x + 16)
    return getfield(x, f)
end
...
const CURLUE_NO_ZONEID = 18 % UInt32
const CURLUE_BAD_FILE_URL = 19 % UInt32
const CURLUE_BAD_FRAGMENT = 20 % UInt32
const CURLUE_BAD_HOSTNAME = 21 % UInt32
const CURLUE_BAD_IPV6 = 22 % UInt32
const CURLUE_BAD_LOGIN = 23 % UInt32
const CURLUE_BAD_PASSWORD = 24 % UInt32
const CURLUE_BAD_PATH = 25 % UInt32
const CURLUE_BAD_QUERY = 26 % UInt32
...
function curl_url_strerror(arg1)
    @ccall libcurl.curl_url_strerror(arg1::CURLUcode)::Ptr{Cchar}
end
...
const CURLHcode = UInt32
const CURLHE_OK = 0 % UInt32
const CURLHE_BADINDEX = 1 % UInt32
const CURLHE_MISSING = 2 % UInt32
const CURLHE_NOHEADERS = 3 % UInt32
const CURLHE_NOREQUEST = 4 % UInt32
const CURLHE_OUT_OF_MEMORY = 5 % UInt32
const CURLHE_BAD_ARGUMENT = 6 % UInt32
const CURLHE_NOT_BUILT_IN = 7 % UInt32

function curl_easy_header(easy, name, index, origin, request, hout)
    @ccall libcurl.curl_easy_header(easy::Ptr{CURL}, name::Ptr{Cchar}, index::Csize_t, origin::Cuint, request::Cint, hout::Ptr{Ptr{curl_header}})::CURLHcode
end

function curl_easy_nextheader(easy, origin, request, prev)
...
const CURLAUTH_AWS_SIGV4 = Culong(1) << 7
...
const CURLHSTS_ENABLE = Clong(1 << 0)

const CURLHSTS_READONLYFILE = Clong(1 << 1)
...
const CURLMIMEOPT_FORMESCAPE = 1 << 0
...
const CURL_VERSION_HSTS = 1 << 28

const CURL_VERSION_GSASL = 1 << 29

const CURL_VERSION_THREADSAFE = 1 << 30
...
const CURLH_TRAILER = 1 << 1

const CURLH_CONNECT = 1 << 2
