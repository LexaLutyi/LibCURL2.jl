...
function Base.getproperty(x::Ptr{curl_fileinfo}, f::Symbol)
    ...
    f === :strings && return Ptr{__JL_Ctag_50}(x + 36)
...
function Base.getproperty(x::Ptr{curl_hstsentry}, f::Symbol)
    ...
    f === :expire && return Ptr{NTuple{18, Cchar}}(x + 9)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{CURLMsg}, f::Symbol)
    ...
    f === :data && return Ptr{__JL_Ctag_49}(x + 8)
    return getfield(x, f)
end
...
const CURL_FORMAT_CURL_OFF_T = "lld"

const CURL_FORMAT_CURL_OFF_TU = "llu"
